...
from ..io.logger import show_arguments, generate_timestamp
...
class CanonicalConfig(TreeConfig):
    ...
    def __init__(self, exp_dir, load_rel_path=None, deploy=True, block_argv=False):
        ...
        # 2. If given, generating is not allowed
            ...
            if not block_argv: assert self._generate_load_rel_path() is None
...
class CanonicalConfigPyTorch(CanonicalConfig):
    ...
    def _init_method(self, **kwargs):
        ...
        # Update dirs.
        for k, v in copy.deepcopy(vars(self.args)).items():
            if k.endswith("dir") and v.endswith("-(TS)"):
                setattr(self.args, k, v[:-len("-(TS)")] + "-(%s)" % generate_timestamp())
        # Check.
