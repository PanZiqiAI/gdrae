...
class BaseModel(nn.Module):
    ...
    def _resume(self, mode, checkpoint_pack=None, verbose=False, **kwargs):
        ...
        # Logging
        if verbose:
            if 'pbar' in self._meters: self._meters['pbar'].write("Loaded from checkpoint '%s'. " % checkpoint_path)
            else: print("Loaded from checkpoint '%s'. " % checkpoint_path)
    ...
    def eval_model(self, **kwargs):
        ...
        if isinstance(self, EpochBatchBaseModel): formatted_counters = ['epoch', 'batch', 'iter']
        elif isinstance(self, IterativeBaseModel): formatted_counters = ['epoch', 'batch', 'step', 'iter']
        ...
        if 'show_cfg_title' not in kwargs.keys():
            ...
            else: kwargs['show_cfg_title'] = kwargs['show_cfg_title'][0]
...
class IterativeBaseModel(BaseModel):
    ...
    def _set_meters(self, **kwargs):
        ...
        super(IterativeBaseModel, self)._set_meters(i={'step': 0, 'epoch': 0, 'batch': -1, 'iter': -1, 'num_cur_epoch': 0}, **kwargs)
        # --------------------------------------------------------------------------------------------------------------
        # Counters - log & chkpt
        # --------------------------------------------------------------------------------------------------------------
    ...
    def _train_procedure(self, **kwargs):
        ...
        packs = self._init_packs()
        # 2. Main
        while self._meters['i']['step'] < self._cfg.args.steps:
            # 1. Train
        ...
        # Save final result
        self._save_checkpoint(self._meters['i']['step'] - 1)
