...
def compute_irs(ground_truth_data, representation_function, random_state, num_bins=20, diff_quantile=0.99, num_train=100, batch_size=16):
    ...
    :param num_bins: Number of bins for discretizing.
    :param diff_quantile: Float value between 0 and 1 to decide what quantile of diffs to select (use 1.0 for the version in the paper).
    :param num_train: Number of points used for training.
    :param batch_size: Batch size for sampling.
    :return: Dict with IRS and number of active dimensions.
    """
    ...
    ys_discrete = histogram_discretize(ys, num_bins=num_bins)
    active_mus = _drop_constant_dims(mus)
    if not active_mus.any(): irs_score = 0.0
    else: irs_score = scalable_disentanglement_score(ys_discrete.T, active_mus.T, diff_quantile)["avg_score"]
    score_dict = {"IRS": irs_score, "num_active_dims": np.sum(active_mus).item()}
    return score_dict
...
def _drop_constant_dims(ys):
    ...
    ys = np.asarray(ys)
    if ys.ndim != 2: raise ValueError("Expecting a matrix.")
...
def scalable_disentanglement_score(gen_factors, latents, diff_quantile=0.99):
    """ Computes IRS scores of a dataset.
        Assumes no noise in X and crossed generative factors (i.e. one sample per combination of gen_factors). Assumes
    each g_i is an equally probable realization of g_i and all g_i are independent.
    :param gen_factors: Numpy array of shape (num samples, num generative factors), matrix of ground truth generative factors.
    :param latents: Numpy array of shape (num samples, num latent dimensions), matrix of latent variables.
    :param diff_quantile: Float value between 0 and 1 to decide what quantile of diffs to select (use 1.0 for the version in the paper).
    :return: Dictionary with IRS scores.
    """
    ...
    if np.sum(max_deviations) > 0.0: avg_score = np.average(disentanglement_scores, weights=max_deviations)
    else: avg_score = np.mean(disentanglement_scores)
    parents = irs_matrix.argmax(axis=1)
    score_dict = {"disentanglement_scores": disentanglement_scores, "avg_score": avg_score, "parents": parents,
                  "IRS_matrix": irs_matrix, "max_deviations": max_deviations}
    return score_dict
